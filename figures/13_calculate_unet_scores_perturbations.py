...
batch_size = 4    
path = os.path.join(gv.BASE_PATH, 'full_cells_fovs_perturbation/train_test_list/**/image_list_with_metadata__with_efficacy_scores_full.csv')
...
if not dfs:
    ...
    base_dir = os.path.join(gv.BASE_PATH, 'full_cells_fovs_perturbation/train_test_list/unet_predictions')
...
def predict_images_unet(dataset,model_path=gv.model_path,model=None,images=range(10),weighted_pcc=False, calculate_on=None):
    """
    Args:
    ...
    for image_index in images:
        ...
        if calculate_on is not None and dataset.df.get_item(image_index,'StructureId/Name') != calculate_on:
            pccs.append(None)
        else:
            slice_by = None
            print("preprocess..")
...
# ds_path = os.path.join(gv.BASE_PATH, "full_cells_fovs_perturbation/train_test_list/unet_predictions/dataset.csv")
for param in params:
    try:
        print(param["model"])
        ...
        pccs = predict_images_unet(dataset,model_path=param["model"],images=range(dataset.df.data.shape[0]),weighted_pcc=False,calculate_on = param["calculate_on"])
        metadata_with_scores_df[param["model"]] = pccs
